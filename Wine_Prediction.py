...
def show_wine_predictions():
    ...
    # If the button is clicked, preprocess the review and predict
    if st.button("Wine Predict"):
        # Define text preprocessing function inside the button click event
        def preprocess_text(text):
            stop_words = set(stopwords.words("english")).union({",", "."})
            lemmatizer = WordNetLemmatizer()
            tokens = word_tokenize(text)  # Tokenize
    ...
    df = pd.read_csv(file_paths["winemag-data-130k-v2.csv"])
